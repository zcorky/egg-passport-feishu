...
const debug = (namespace, ...args) => {
  const isDEBUGMode = process.env.DEBUG_EGG_PASSPORT_FEISHU === 'true';
  if (!isDEBUGMode) return;

  console.log(`[${new Date()}][egg_passport_feishu][debug][namespace: ${namespace}]`, ...args);
};
...
function Strategy(options, verify) {
  ...
  this._oauth2.getOAuthAccessToken = function(code, params, callback) {
    self.getAppAccessToken()
      .then(token => {
        debug('_oauth2.getOAuthAccessToken.getAppAccessToken.token', token);
        ...
        data.code = code;
        request
          .post(self._oauth2._getAccessTokenUrl(), {
            headers: {
          ...
          .json(data)
          .then(results => {
            debug('_oauth2.getOAuthAccessToken.results', results);
...
Strategy.prototype.getAppAccessToken = function() {
  ...
  return new Promise((resolve, reject) => {
    request.post(this._appTokenURL).json(data)
      .then(results => {
        debug('getAppAccessToken.results', results);
...
Strategy.prototype.userProfile = function(accessToken, done) {
  request.get({
    ...
    .then(results => {
      debug('userProfile.results', results);
